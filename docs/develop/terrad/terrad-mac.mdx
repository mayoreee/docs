...
This guide is for developers who want to install JMESd and interact with JMES Core without running a full node. If you want to run a full node or join a network, use the documentation on how to [run a full JMES node](../../full-node/run-a-full-terra-node/README.mdx).
