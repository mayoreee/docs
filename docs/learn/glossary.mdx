...
Governance is the democratic process that allows users and [validators](#validator) to make changes to the JMES protocol. Community members submit, vote, and implement proposals. One staked [Luna](#luna) is equal to one vote.
...
The native staking token of the JMES protocol. Luna is also used as a governance token. [Delegators](#delegator) can stake Luna to receive rewards.
