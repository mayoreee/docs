...
- You have completed [how to run a full JMES node](../run-a-full-terra-node/README.mdx), which outlines how to install, connect, and configure a node.
- You are familiar with [JMESd](../../develop/terrad/install-terrad.mdx).
...
For the testnet, use [the faucet](http://faucet.jmes.cloud/) to send bJMES to your wallet. If you are on mainnet, send funds from an existing wallet. 1-3 bJMES are sufficient for most setup processes.
...
```
http://explorer.jmes.cloud/faucet/:3002/credit?address=<Insert_Wallet_Address_Here>&denom=ujmes
```
...
```bash
jmesd query tendermint-validator-set | grep "$(jmesd tendermint show-validator)"
```
