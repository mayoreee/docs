...
Running a full JMES node is a resource-intensive process that requires a persistent server. If you want to use JMES without downloading the entire blockchain, use [Keplr Wallet](https://station.terra.money/). If you want to set up a local, WASM-enabled, private testnet for smart contracts, visit [install LocalJMES](../../develop/localterra/README.mdx).
...
| [`jmes-888`](join-a-network.mdx#join-a-public-network)                    | 2 (+2 threads)| 16 GB   | 500 GB (SSD 1000 MB/s R/W) | 150 Mbps  |
| [`localjmes`](https://github.com/jmesworld/LocalJMES)               | 2             | 4 GB    | 20 GB (SSD 500 MB/s R/W)   |  N/A      |
...
jmesd uses the following TCP ports. Toggle their settings to match your environment.
...
- `1317`: The default port for the [Lite Client Daemon](../../develop/guides/start-lcd.mdx) (LCD), which can be executed by `jmesd rest-server`. The LCD provides an HTTP RESTful API layer to allow applications and services to interact with your JMESd instance through RPC. For usage examples, see [JMES REST API](https://phoenix-lcd.terra.dev/swagger/). You don't need to open this port unless you have use for it.
...
- `26657`: The default port for the RPC protocol. Because this port is used for querying and sending transactions, it must be open for serving queries from jmesd.
