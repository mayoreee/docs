...
The following information describes the most important node configuration settings found in the `~/.jmes/config/` directory. It is recommended that you update these settings with your own information.

<details> 
<summary> Structure of `.jmes/config` </summary>
<p>

```bash
~/.jmes/config
│-- addrbook.json                       # a registry of peers to connect to
│-- app.toml                            # jmesd configuration file
│-- client.toml                         # configurations for the cli wallet (ex jmescli)
...
```bash
jmesd init <your_custom_moniker> # ex., jmesd init validator-joes-node
```
...
minimum-gas-prices = "0.01133ubJMES"
...
5. Restart the service via `systemctl restart jmesd`. Once restarted, the LCD will be available (by default on port `127.0.0.1:1317`).
