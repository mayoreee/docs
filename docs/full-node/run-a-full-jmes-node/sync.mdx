...
1. To install the latest version `v2.0.0-rc.0` of jmesd, [navigate into your `core` directory](./build-jmes-core.mdx#get-the-jmes-core-source-code) and execute the following commands in your terminal:
...
1. To switch to version `v2.0.0` of jmesd, [change into your `core` directory](./build-jmes-core.mdx#get-the-jmes-core-source-code) and execute the following commands in your terminal:
...
```
wget https://phoenix-genesis.s3.us-west-1.amazonaws.com/genesis.json -O ~/.jmes/config/genesis.json
```

3. Next, download the `addrbook.json` file containing a list of stable persistent peers into the `~/.jmes/config/` directory:

```
wget https://snapshots1.polkachu.com/addrbook/jmes/addrbook.json -O ~/.jmes/config/addrbook.json
```
